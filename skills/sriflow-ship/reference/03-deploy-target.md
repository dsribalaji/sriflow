...
| 5. docker (Dockerfile)
| 6. npm (package.json with "bin" field)
| 7. pip (setup.py / pyproject.toml)
| 8. homebrew (Formula/*.rb)
| 9. go-install (go.mod with main package)
| 10. binary-release (fallback — no platform config)
...
|---------|---------------|-------|
| vercel | `npx vercel --prod` | Requires Vercel CLI or npx |
| fly | `fly deploy` | Reads fly.toml |
| railway | `railway up` | Deploys to linked Railway project |
| github-actions | push triggers workflow | Poll CI via `gh run list` |
| docker | `docker build -t <project>:<sha> . && docker push <registry>/<project>:<sha>` | Must know registry |
| npm | `npm publish` or `npm publish --access public` | For CLI tools packaged on npm |
| pip | `python -m build && twine upload dist/*` | Python packages |
| homebrew | Create/update formula in tap | Requires tap repo access |
| go-install | `go install <module>@latest` | Go binaries |
| binary-release | Create GitHub release with compiled binaries | Cross-platform builds |
| custom | user-provided | Collected via D1b |
