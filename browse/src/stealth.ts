 ...
 * SRIFLOW_STEALTH=extended, off by default) DOES fake plugins — that mode
 ...
 *      detectors flag.
 *   4. Report per-install hardware values via SRIFLOW_HW_CONCURRENCY /
 *      SRIFLOW_DEVICE_MEMORY env vars (set by gbd at startup via
...
// the UA-CH cmdline switch in buildGStackLaunchArgs (which reads SRIFLOW_PLATFORM
 ...
 *   3. EXTENDED_STEALTH_SCRIPT — only when SRIFLOW_STEALTH=extended (off by
...
/**
 * Build the `--sriflow-*=` cmdline switches that the Pack 1 Chromium
 * patches read (webgl-vendor-spoof, ua-client-hints-stealth, worker-
 * navigator-stealth). Values come from the SRIFLOW_* env vars that
 ...
 *
 * TRUSTED-SOURCE ONLY. These SRIFLOW_* values are populated by gbd from
 ...
 *   SRIFLOW_GPU_VENDOR        → --sriflow-gpu-vendor        → webgl-vendor-spoof.patch
 *   SRIFLOW_GPU_RENDERER      → --sriflow-gpu-renderer      → webgl-vendor-spoof.patch
 *   SRIFLOW_PLATFORM          → --sriflow-ua-platform       → ua-client-hints-stealth.patch
 *                              (maps MacARM/MacIntel → "macOS")
 *   SRIFLOW_GPU_CHIPSET       → --sriflow-ua-model          → ua-client-hints-stealth.patch
 *   SRIFLOW_HW_CONCURRENCY    → --sriflow-hw-concurrency    → worker-navigator-stealth.patch
 *   SRIFLOW_DEVICE_MEMORY     → --sriflow-device-memory     → worker-navigator-stealth.patch
 */
export function buildGStackLaunchArgs(): string[] {
  ...
  if (vendor) args.push(`--sriflow-gpu-vendor=${vendor}`);

  const renderer = env.SRIFLOW_GPU_RENDERER;
  if (renderer) args.push(`--sriflow-gpu-renderer=${renderer}`);
  ...
  if (chipset) args.push(`--sriflow-ua-model=${chipset}`);

  const hw = env.SRIFLOW_HW_CONCURRENCY;
  if (hw) args.push(`--sriflow-hw-concurrency=${hw}`);

  const memory = env.SRIFLOW_DEVICE_MEMORY;
  if (memory) args.push(`--sriflow-device-memory=${memory}`);
  ...
  //
  // OPT-IN (off by default): only emitted when SRIFLOW_CDP_STEALTH is
  // on/1/true. This switch is read by a C++ patch that only exists in
  // gbrowser builds; gbd opts in by exporting SRIFLOW_CDP_STEALTH=on. Stock
  ...
  // unless SRIFLOW_CDP_STEALTH=off, which contradicted this very comment.)
