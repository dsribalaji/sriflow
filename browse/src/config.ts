 ...
 *   2. git rev-parse --show-toplevel → projectDir/.sriflow/
...
export function resolveConfig(
  ...
  if (env.BROWSE_STATE_FILE) {
    ...
    projectDir = path.dirname(stateDir); // parent of .sriflow/
  } else {
    projectDir = getGitRoot() || process.cwd();
    stateDir = path.join(projectDir, '.sriflow');
...
/**
 * Create the .sriflow/ state directory if it doesn't exist.
...
export function ensureStateDir(config: BrowseConfig): void {
  ...
  // Ensure .sriflow/ is in the project's .gitignore
  const gitignorePath = path.join(config.projectDir, '.gitignore');
  try {
    const content = fs.readFileSync(gitignorePath, 'utf-8');
    if (!content.match(/^\.sriflow\/?$/m)) {
      const separator = content.endsWith('\n') ? '' : '\n';
      fs.appendFileSync(gitignorePath, `${separator}.sriflow/\n`);
    }
  } catch (err: any) {
    if (err.code !== 'ENOENT') {
...
/**
 * Resolve the sriflow home directory.
 *
 * Honors the existing convention used by telemetry.ts and domain-skills.ts:
 *   1. SRIFLOW_HOME env (explicit override)
 *   2. $HOME/.sriflow (default)
 */
export function resolveSriflowHome(): string {
  return process.env.SRIFLOW_HOME || path.join(os.homedir(), '.sriflow');
}
