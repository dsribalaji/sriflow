...
// trim: cookie-import-browser, cookie-picker-routes removed for sriflow (solo use)
  ...
  switch (command) {
    ...
    case 'load-html': {
      ...
      if (fromFilePayload) {
        ...
        if (Buffer.byteLength(fromFilePayload.html, 'utf8') > MAX_BYTES) {
          throw new Error(
            ...
            'Raise with SRIFLOW_BROWSE_MAX_HTML_BYTES=<N>.'
          );
        }
      ...
      if (stat.size > MAX_BYTES) {
        throw new Error(
          `load-html: file too large (${stat.size} bytes > ${MAX_BYTES} cap). Raise with SRIFLOW_BROWSE_MAX_HTML_BYTES=<N> or split the HTML.`
        );
      }
    ...
    // trim: cookie-import, cookie-import-browser commands removed for sriflow (solo use)
      ...
      if (doSticky) {
        const stickyCount = await page.evaluate(() => {
          ...
          for (const { el, top, width, height } of stickyEls) {
            ...
            // Skip the sriflow control indicator
            if ((el as HTMLElement).id === 'sriflow-ctrl') continue;
