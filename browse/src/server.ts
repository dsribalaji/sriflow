/**
 * sriflow browse server — persistent Chromium daemon
 *
 * Architecture:
 ...
 *
 * State:
 *   State file: <project-root>/.sriflow/browse.json (set via BROWSE_STATE_FILE env)
 *   Log files:  <project-root>/.sriflow/browse-{console,network,dialog}.log
...
// trim: cookie-picker-routes, sidebar-utils removed for sriflow (solo use)
...
// trim: security-sidecar removed for sriflow (solo use, no ML classifier)
import { writeSecureFile, mkdirSecure } from './file-permissions';
import { handleSnapshot, SNAPSHOT_FLAGS } from './snapshot';
// trim: token-registry removed for sriflow (no auth tokens, localhost only)
...
// trim: audit removed for sriflow (no audit logging needed for solo use)
import { inspectElement, modifyStyle, resetModifications, getModificationHistory, detachSession, type InspectorResult } from './cdp-inspector';
// Bun.spawn used instead of child_process.spawn (compiled bun binaries
// fail posix_spawn on all executables including /bin/bash)
import { safeUnlink, safeUnlinkQuiet, safeKill } from './error-handling';
// trim: terminal-agent-control removed for sriflow (no terminal agent)
import { isProcessAlive } from './error-handling';
import { sanitizeBody, stripLoneSurrogateEscapes } from './sanitize';
// trim: socks-bridge removed for sriflow (no proxy support)
// trim: proxy-config, proxy-redact removed for sriflow (no proxy support)
// trim: xvfb removed for sriflow (headless only)
// trim: tunnel-denial-log removed for sriflow (no tunnel)
// trim: sse-session-cookie removed for sriflow (solo use, no SSE auth)
// trim: pty-session-cookie/lease removed for sriflow (no terminal agent)
...
//
// trim: tunnel state removed for sriflow (solo use, no tunnel)
...
export interface ServerConfig {
  ...
  startTime: number;
  /**
   * Overlay hook. Runs AFTER auth resolution and BEFORE route dispatch.
   */
  beforeRoute?: (req: Request, surface: Surface, auth: null) => Promise<Response | null>;
}
...
/**
 * Build a ServerConfig-shaped object from process.env. Used by sriflow's
...
// trim: tunnel/ngrok functions removed for sriflow (solo use, no tunnel)

/** Stub TokenInfo type for sriflow (solo use, no auth tokens) */
type TokenInfo = { clientId: string; scopes: string[]; token: string } | null;

/** Stub getTokenInfo for sriflow (solo use, no auth tokens) — always returns root token */
function getTokenInfo(_req: Request): TokenInfo {
...
async function handleCommandInternalImpl(
  ...
  // trim: scope/rate/domain checks removed for sriflow (solo use, localhost only)
  ...
  try {
    ...
    // trim: audit logging removed for sriflow (solo use)
...
export function buildFetchHandler(cfg: ServerConfig): ServerHandle {
  ...
  // Factory-scoped validateAuth. For sriflow solo use, always passes (no auth required).
  function validateAuth(_req: Request): boolean {
    return true;
  }

  // Factory-scoped shutdown. Closes the cfg-provided browserManager.
  ...
  const makeFetchHandler = (surface: Surface) => async (req: Request): Promise<Response> => {
    ...
    // trim: tunnel surface filter removed for sriflow (solo use, localhost only)

    // beforeRoute overlay hook
    if (beforeRoute) {
      const overlayResp = await beforeRoute(req, surface, null);
      if (overlayResp) return overlayResp;
    }

    // GET /connect — alive probe
    if (url.pathname === '/connect' && req.method === 'GET') {
      ...
      // trim: /cookie-picker removed for sriflow (solo use)

      // Welcome page — served when GStack Browser launches in headed mode
      if (url.pathname === '/welcome') {
        const welcomePath = (() => {
          // Gate SRIFLOW_SLUG on a strict regex BEFORE interpolating it into
          // the filesystem path. Without this, a slug like "../../etc/passwd"
          // would resolve to ~/.sriflow/projects/../../etc/passwd/... — path
          ...
          // Fallback: built-in welcome page from sriflow install.  Reject
          // SKILL_ROOT values containing '..' for the same defense-in-depth
          // reason as the SRIFLOW_SLUG regex above.  Not exploitable today
        ...
        return new Response(
          `<!DOCTYPE html><html><head><title>SriFlow Browser</title>
          ...
          <body><div class="msg"><div class="gold">◈</div><p>SriFlow Browser ready.</p><p style="font-size:.85em">Waiting for commands from Claude Code.</p></div></body></html>`,
      ...
      // trim: /pty-inject-scan removed for sriflow (no terminal agent)

      // trim: /connect POST, /token, /agents, /pair, /tunnel/start, /sse-session removed for sriflow (solo use, no auth tokens)
      ...
      if (url.pathname === '/command' && req.method === 'POST') {
        ...
        // trim: tunnel command filtering removed for sriflow (solo use)
...
// trim: __resetShuttingDown removed for sriflow (test helper, not needed)

// Auto-kickoff only when this module is the entry point.
if (import.meta.main) {
  start().catch((err) => {
