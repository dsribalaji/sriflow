...
const SECURITY_DIR = path.join(os.homedir(), '.sriflow', 'security');
 ...
 * ~/.sriflow/security/device-salt (0600). Random per-device, prevents rainbow
...
/**
 * Try to locate the sriflow-telemetry-log binary. Resolution order matches
 * the existing skill preamble pattern (never relies on PATH — packaged
 * binary layouts can break that).
 *
 * Order:
 *  1. ~/.claude/skills/sriflow/bin/sriflow-telemetry-log  (global install)
 *  2. .claude/skills/sriflow/bin/sriflow-telemetry-log    (symlinked dev)
 *  3. bin/sriflow-telemetry-log                          (in-repo dev)
 */
function findTelemetryBinary(): string | null {
  const candidates = [
    path.join(os.homedir(), '.claude', 'skills', 'sriflow', 'bin', 'sriflow-telemetry-log'),
    path.resolve(process.cwd(), '.claude', 'skills', 'sriflow', 'bin', 'sriflow-telemetry-log'),
    path.resolve(process.cwd(), 'bin', 'sriflow-telemetry-log'),
  ];
 ...
 * SRIFLOW_*_BIN override pattern from `browse/src/claude-bin.ts:resolveClaudeCommand`
 * (introduced in v1.24.0.0 #1252) so users on WSL/MSYS2/non-default Git Bash
 * installs can redirect.
 *
 * Override precedence:
 *   1. SRIFLOW_BASH_BIN (or BASH_BIN) — absolute path or PATH-resolvable command.
...
export function resolveBashBinary(env: NodeJS.ProcessEnv = process.env): string | null {
  ...
  const override = (env.SRIFLOW_BASH_BIN ?? env.BASH_BIN)?.trim();
  if (override) {
 ...
 * Win32: wraps in bash explicitly. `sriflow-telemetry-log` is a shell script
 ...
 * Bun.which + SRIFLOW_*_BIN override pattern as claude-bin.ts.
 *
 * Returns null when bash can't be resolved on Windows (rare — Git Bash ships
 * with the standard sriflow install path). Caller skips spawn; the local
...
/**
 * Fire-and-forget subprocess invocation of sriflow-telemetry-log with the
...
// per-tab file under ~/.sriflow/security/decisions/, sidebar-agent polls
