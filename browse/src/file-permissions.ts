/**
 * Cross-platform file permission restriction for sensitive sriflow state.
 *
 * Why this exists
 * ----------------
 * POSIX mode bits (`0o600` for files, `0o700` for dirs) are how sriflow marks
 ...
 * install, sensitive sriflow state files inherit whatever ACL the parent
...
function warnIcaclsFailure(fsPath: string, err: unknown): void {
  ...
  console.warn(
    `[sriflow] Failed to restrict Windows ACL on ${fsPath}: ${msg}\n` +
