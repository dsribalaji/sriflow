...
/**
 * Detect whether SRIFLOW_CHROMIUM_PATH points at a custom Chromium build that
 * already bakes the sriflow extension in as a component extension (e.g.,
 ...
 *
 * Resolution:
 *   1. SRIFLOW_CHROMIUM_KIND === 'custom-extension-baked' (preferred, explicit)
 *   2. SRIFLOW_CHROMIUM_PATH path substring contains 'GBrowser' or 'gbrowser'
 *      (fallback for callers that only set the path)
 */
export function isCustomChromium(): boolean {
  if (process.env.SRIFLOW_CHROMIUM_KIND === 'custom-extension-baked') return true;
...
export function shouldEnableChromiumSandbox(): boolean {
  ...
  // so /qa hangs without --no-sandbox). Setting SRIFLOW_CHROMIUM_NO_SANDBOX=1
  // forces the sandbox off without changing the default for everyone else.
  // See #1562.
  if (process.env.SRIFLOW_CHROMIUM_NO_SANDBOX === '1') return false;
...
export async function resolveDisconnectCause(browser: Browser | null): Promise<'clean' | 'crash'> {
  // Playwright 1.61 removed browser.process() — can't inspect child exit code.
  // For a local daemon without gbd supervisor, always treat disconnect as clean.
  return 'clean';
}
...
export class BrowserManager {
  ...
  /**
   * Find the sriflow Chrome extension directory.
  ...
  private findExtensionPath(): string | null {
    ...
    const candidates = [
      ...
      // Global sriflow install
      path.join(process.env.HOME || '', '.claude', 'skills', 'sriflow', 'extension'),
      // Git repo root (detected via BROWSE_STATE_FILE location)
      (() => {
        const stateFile = process.env.BROWSE_STATE_FILE || '';
        if (stateFile) {
          const repoRoot = path.resolve(path.dirname(stateFile), '..');
          return path.join(repoRoot, '.claude', 'skills', 'sriflow', 'extension');
        }
        return '';
      })(),
    ].filter(Boolean);
  ...
  // ─── Headed Mode ─────────────────────────────────────────────
  /**
   * Launch Playwright's bundled Chromium in headed mode with the sriflow
  ...
  async launchHeaded(authToken?: string): Promise<void> {
    ...
    // Find the sriflow extension directory for auto-loading
    ...
    if (extensionPath) {
      ...
      // the extension is component-baked — it reads ~/.sriflow/.auth.json at
      // startup to learn how to call the daemon).
      // Write to ~/.sriflow/.auth.json (not the extension dir, which may be read-only
      // in .app bundles and breaks codesigning).
      if (authToken) {
        const fs = require('fs');
        const path = require('path');
        const sriflowDir = path.join(process.env.HOME || '/tmp', '.sriflow');
        mkdirSecure(sriflowDir);
        const authFile = path.join(sriflowDir, '.auth.json');
        try {
    ...
    // single-instance CLI check for sriflow.
    cleanSingletonLocks(userDataDir);

    // Support custom Chromium binary via SRIFLOW_CHROMIUM_PATH env var.
    ...
    try {
      ...
      if (fs.existsSync(chromePlist)) {
        ...
        const iconCandidates = [
          ...
          path.join(process.env.HOME || '', '.claude', 'skills', 'sriflow', 'scripts', 'app', 'icon.icns'), // global install
        ];
    ...
    const indicatorScript = () => {
      const injectIndicator = () => {
        if (document.getElementById('sriflow-ctrl')) return;
